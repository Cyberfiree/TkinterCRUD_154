...
def fill_inputs_from_table(event):
    try:
        ...
        selected_data = tree.item(selected_item)['values']  # Mendapatkan data dari item yang dipilih
        
        selected_id.set(selected_data[0])  # Mengisi ID yang dipilih
        nama_var.set(selected_data[1])  # Mengisi nama siswa
        biologi_var.set(selected_data[2])  # Mengisi nilai biologi
        fisika_var.set(selected_data[3])  # Mengisi nilai fisika
        inggris_var.set(selected_data[4])  # Mengisi nilai inggris
    except IndexError:  # Jika tidak ada data yang dipilih
        messagebox.showerror("Error", "Pilih data yg valid!")  # Menampilkan pesan error

# Inisialisasi database saat program pertama kali dijalankan
create_database()

# Membuat GUI dengan tkinter
root = Tk()
root.title("Prediksi Fakultas Siswa")  # Judul window
root.configure(bg="#ff7477")  # Mengatur warna latar belakang

# Variabel tkinter untuk inputan
nama_var = StringVar()
...
# Menambahkan label dan entry (input) untuk setiap data siswa
...
# Membuat tombol untuk menambah, mengubah, dan menghapus data
...
# Menyesuaikan posisi kolom agar berada di tengah
for col in columns:
    tree.column(col, anchor="center")

# Mengatur event handler untuk memilih data dari tabel
